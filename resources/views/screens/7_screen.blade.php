<div class="screen--7__bg">
    <div class="container-fluid container-max-width screen--7 py-5">
        <div class="row">
            <div class="col-xl-7 screen--7__col-xl-7 py-lg-5">
                <div class="screen--7__content">
                    <p class="h1 screen--7__display-4 text-white mb-5">
                        Powered by Fast
                        <span class="block-line bg-primary"></span> <br>
                        and Secure Blockchains
                    </p>
                    <p class="h3 screen--7__desc font-weight-normal text-white mb-5 pb-4">
                        ET Protocol is using EOS.IO, one of the fastest <br>
                        blockchain protocols with low transaction fees, <br>
                        and Hyperledger by IBM
                    </p>
                    <a href="#" class="btn btn-primary btn--main">
                        read tech paper
                    </a>
                </div>
            </div>
            <div class="col pt-5 mt-3">
                <div class="px-5 my-3 my-xl-0 py-3 screen--7__box bg-white">
                    <div class="d-flex flex-column flex-sm-row">
                        <div class="col-auto d-flex align-items-start justify-content-center pr-4">
                            <div class="mx-3">
                                <img width="50" src="/img/screen7/1.png" alt="1">
                            </div>
                        </div>
                        <div class="col-auto col-sm">
                            <p class="h1 lh-1_4 mb-2 font-weight-normal text-black-50">
                                EOS
                            </p>
                            <p class="text-muted h5 text-black-50">
                                EOS is a smart contract platform for the deployment of decentralized applications that supports
                                millions of transactions per second.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="px-5 my-3 my-xl-0 py-3 screen--7__box bg-white">
                    <div class="d-flex flex-column flex-sm-row">
                        <div class="col-auto d-flex align-items-start justify-content-center pr-4">
                            <div>
                                <img width="74" src="/img/screen7/2.png" alt="1">
                            </div>
                        </div>
                        <div class="col-auto col-sm">
                            <p class="h1 lh-1_4 mb-2 font-weight-normal text-black-50">
                                Hyperledger
                            </p>
                            <p class="text-muted h5 text-black-50">
                                Hyperledger is an umbrella project of open source private blockchains supported by IBM.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>





